    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            with h5py.File("/n/holystore01/LABS/itc_lab/Lab/Camels/3D_grids_128_z/3D_CV_128.h5","r") as h5:
                mass_mstar = np.array(h5["mstar_z="+self.z_star][:24])
                mass_cdm = np.array(h5["mcdm_z="+self.z_cdm][:24])
                if self.return_params:
                    params=np.array(h5["params"][:24])
                else:
                    params=None
            #remove sim 2,8,17
            ...
            data = AstroDataset(mass_mstar, mass_cdm,params, ndim=self.ndim, do_crop=self.do_crop, crop=self.cropsize,pad=self.padsize, aug_shift=self.aug_shift,transform=self.train_transforms)
        ...
        if stage == "test" or stage is None:
            ...
            self.test_data = AstroDataset(mass_mstar, mass_cdm,params, ndim=self.ndim, do_crop=self.do_crop,crop=self.cropsize, pad=self.padsize, aug_shift=False,transform=self.test_transforms)
...
def get_dataset_3D_128_CV_CV_z(
    z_star="0.0",
    z_cdm="0.0",
    num_workers=1,
    batch_size=1,
